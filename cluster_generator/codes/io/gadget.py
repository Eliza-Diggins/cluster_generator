...
from cluster_generator.codes._models import AutoValidatedBaseModel
from cluster_generator.codes._types import UnytArray, UnytQuantity, UnytUnit
...
class GenericGadget2_Header(AutoValidatedBaseModel):
    ...
    Attributes
    ----------
    HEADER_FLOAT_TYPE : dtype
        The default float type for header values.
    HEADER_UINT_TYPE : dtype
        The default uint type for header values.
    HEADER_INT_TYPE : dtype
        The default int type for header values.
    HEADER_ORDER : List[str]
        The order of header keys and values. Only necessary in binary format where the order of the header matters.
    HEADER_SIZE : int
        The number of bytes this header takes up.
    N_PARTICLE_TYPES : int
        The number of particle types recognized by this header.
    NumPart_ThisFile : array of uint
        The number of particles of each type in this file.
    NumPart_Total : NDArray[HEADER_UINT_TYPE]
        The total number of particles of each type across the entire simulation.
    BoxSize : HEADER_FLOAT_TYPE
        The size of the simulation box in comoving units.
    MassTable : array of float
        The fixed mass of each particle type. If 0, then the masses are stored in a separate block.
    Time : HEADER_FLOAT_TYPE
        The time of this snapshot.
    Redshift : HEADER_FLOAT_TYPE
        The redshift of this snapshot.
    Flag_Sfr : HEADER_INT_TYPE
        Star formation flag (1 if star formation is included, 0 otherwise).
    Flag_Feedback : HEADER_INT_TYPE
        Feedback flag (1 if feedback is included, 0 otherwise).
    Flag_Cooling : HEADER_INT_TYPE
        Cooling flag (1 if cooling is included, 0 otherwise).
    NumFilesPerSnapshot : HEADER_INT_TYPE
        Number of files in which this snapshot is split.
    Omega0 : HEADER_FLOAT_TYPE
        The matter density parameter at redshift 0.
    OmegaLambda : HEADER_FLOAT_TYPE
        The cosmological constant density parameter at redshift 0.
    HubbleParam : HEADER_FLOAT_TYPE
        The Hubble constant in units of 100 km/s/Mpc.
    Flag_StellarAge : HEADER_INT_TYPE
        Stellar age flag (1 if stellar ages are tracked, 0 otherwise).
    Flag_Metals : HEADER_INT_TYPE
        Metals flag (1 if metallicity is tracked, 0 otherwise).
    NumPart_Total_HW : array of uint
        The high word of the total number of particles of each type.
    Flag_Entropy_ICs : HEADER_INT_TYPE
        Entropy flag (1 if entropy is used as an initial condition, 0 otherwise).
    ...
    # Class specific variables to be specified as fixed values in subclasses.
    # These control the output datatypes of the header class.
    HEADER_FLOAT_TYPE: ClassVar[str | np.dtype] = _MissingParameter()
    HEADER_UINT_TYPE: ClassVar[str | np.dtype] = _MissingParameter()
    HEADER_INT_TYPE: ClassVar[str | np.dtype] = _MissingParameter()
    HEADER_ORDER: ClassVar[list[str]] = _MissingParameter()
    HEADER_SIZE: ClassVar[int] = _MissingParameter()
    N_PARTICLE_TYPES: ClassVar[int] = _MissingParameter()

    # Unit fields. These can be fixed or left for the user to specify depending on the subclass.
    # Generally these may be inferred from context and should have code-specific defaults.
    # type hints should annotate with equivalent units.
    UNIT_LENGTH: UnytUnit["kpc"] = Field(default="kpc", validate_default=True)
    UNIT_TIME: UnytUnit["Myr"] = Field(default="Myr", validate_default=True)
    UNIT_MASS: UnytUnit["Msun"] = Field(default="Msun", validate_default=True)

    # Header fields.
    # All of these are pydantic validated. The dtype should be f,i,u and will dictate what
    # datatype coercion is used when writing the header to file.
    NumPart_ThisFile: list[int] = Field(dtype="u", binary_flag="Npart", size=6)
    NumPart_Total: list[int] = Field(dtype="i", binary_flag="Nall", size=6)
    BoxSize: UnytQuantity[float, unyt.Unit("kpc")] = Field(
        dtype="f", binary_flag="BoxSize"
    )
    MassTable: UnytArray[float, unyt.Unit("Msun")] = Field(
        dtype="f",
        binary_flag="Massarr",
        size=6,
        default_factory=lambda: unyt.unyt_array(np.zeros(6), "Msun"),
    )
    Time: UnytQuantity[float, unyt.Unit("Myr")] = Field(
        dtype="f", binary_flag="Time", default=0.0
    )
    Redshift: float = Field(dtype="f", binary_flag="Redshift", default=0.0)
    Flag_Sfr: Literal[0, 1] = Field(dtype="i", binary_flag="FlagSfr", default=0)
    Flag_Feedback: Literal[0, 1] = Field(
        dtype="i", binary_flag="FlagFeedback", default=0
    )
    Flag_Cooling: Literal[0, 1] = Field(dtype="i", binary_flag="FlagCooling", default=0)
    NumFilesPerSnapshot: Literal[0, 1] = Field(
        dtype="i", binary_flag="NumFiles", default=1
    )
    Omega0: float = Field(dtype="f", binary_flag="Omega0", default=0.0)
    OmegaLambda: float = Field(dtype="f", binary_flag="OmegaLambda", default=0.0)
    HubbleParam: float = Field(dtype="f", binary_flag="HubbleParam", default=0.0)
    Flag_StellarAge: Literal[0, 1] = Field(dtype="i", binary_flag="FlagAge", default=0)
    Flag_Metals: Literal[0, 1] = Field(dtype="i", binary_flag="FlagMetals", default=0)
    NumPart_Total_HW: list[int] = Field(
        dtype="i",
        binary_flag="NallHW",
        size=6,
        default_factory=lambda: np.zeros(6, dtype=np.uint32),
    )
    Flag_Entropy_ICs: Literal[0, 1] = Field(
        dtype="i", binary_flag="flag_entr_ics", default=0
    )
    ...
    @staticmethod
    def _unit_validation(cls, value):
        return validate_units(cls, value)

    @model_validator(mode="after")
    def unit_validation(cls, value):
        return cls._unit_validation(cls, value)
    ...
    def unit_system(self) -> unyt.UnitSystem:
        ...
        return unyt.UnitSystem(
            "HEADER",
            unyt.Unit(self.UNIT_LENGTH),
            unyt.Unit(self.UNIT_MASS),
            unyt.Unit(self.UNIT_TIME),
            "K",
            "rad",
        )

    @classmethod
    def _dtype_map(cls) -> dict[str, str | np.dtype]:
        return {
            "f": cls.HEADER_FLOAT_TYPE,
            "u": cls.HEADER_UINT_TYPE,
            "i": cls.HEADER_INT_TYPE,
        }
    ...
    def generate_field_mapping(cls) -> dict[str, str]:
        ...
        for field_name, field_info in cls.__fields__.items():
            ...
            try:
                binary_flag = field_info.json_schema_extra["binary_flag"]
                _map[binary_flag] = field_name
            except KeyError:
                continue
        return _map

    def __repr__(self) -> str:
        """Return a detailed string representation of the GenericGadget2_Header.
        ...
        -------
        str
            A string representation of the GenericGadget2_Header instance.
        """
        header_details = []
        for field_name, field_info in self.__class__.__fields__.items():
            if field_info.json_schema_extra is None:
                # skip non-true header fields.
                continue
            field_value = getattr(self, field_name)
            header_details.append(f"{field_name}: {field_value}")
        return "GenericGadget2_Header(\n  " + "\n  ".join(header_details) + "\n)"

    def __str__(self) -> str:
        """Provide a one-line summary of the GenericGadget2_Header.
        ...
        return f"<GenericGadget2_Header: {num_particles} particles, BoxSize = {self.BoxSize}>"
    ...
    @classmethod
    def _get_header_struct(cls: Type["GenericGadget2_Header"]) -> np.dtype:
        ...
        for field_name, field_info in cls.__fields__.items():
            if field_info.json_schema_extra is None:
                continue  # This is not a true header field.

            metadata = field_info.json_schema_extra
            try:
                dtype = cls._dtype_map[metadata["dtype"]]
                binary_flag = metadata["binary_flag"]
                size = metadata.get("size", 1)
            except KeyError:
                raise TypeError(
                    f"Failed to load {field_name} struct because class definition is inconsistent."
                )
    ...
    def load_header(cls, path: str | Path, format: Literal["binary", "hdf5"]) -> Self:
        ...
        GenericGadget2_Header
            An instance of `GenericGadget2_Header` containing the metadata read from the file header. This
        ...
        .. code-block:: python
            ...
            >>> header = GenericGadget2_Header.load_header("simulation_file.g2", format="binary")
    ...
    def write_header(
        ...
        **kwargs : dict, optional
        ...
        path = prepare_path(path, overwrite=overwrite)

        # Generate the header values in a ready-to-write dict.
        header_dict = self._get_field_values(format=format)
    ...
    @classmethod
    def _load_header_binary(cls, path: Path) -> "GenericGadget2_Header":
    ...
    @classmethod
    def _load_header_hdf5(cls, path: Path) -> "GenericGadget2_Header":
    ...
    def _get_field_values(self, format: str = "binary") -> dict[str, Any]:
        ...
        for field_name, field_info in self.__class__.__fields__.items():
            if field_info.json_schema_extra is None:
                continue  # Skip the non-header (unit) fields.

            metadata = field_info.json_schema_extra

            # load the field value and coerce units and dtype.
            field_value = self.__getattribute__(field_name)
            dtype, size = self.__class__._dtype_map()[metadata["dtype"]], metadata.get(
                "size", 1
            )

            if hasattr(field_value, "units"):
                field_value = field_value.in_base(self.unit_system).d

            header_dict[field_name] = field_value

            if size > 1:
                try:
                    header_dict[field_name] = np.reshape(
                        header_dict[field_name], (size,)
                    )
                except Exception:
                    raise ValueError(
                        f"Field {field_name} could not be coerced to have shape ({size},)."
                    )

            header_dict[field_name] = np.asarray(header_dict[field_name], dtype=dtype)
    ...
    @classmethod
    def from_particles(
        cls: Type["GenericGadget2_Header"],
        particles: ClusterParticles,
        particle_registry: "GenericGadget2_ParticleRegistry",
        cosmology: "Cosmology" = None,
        boxsize_buffer: float = 0.25,
        **kwargs,
    ) -> "GenericGadget2_Header":
        """Create a `GenericGadget2_Header` instance from particle data and a particle
        registry.

        This method constructs a `GenericGadget2_Header` by analyzing particle data provided in a
        ...
        **kwargs : dict, optional
            Additional keyword arguments that can be passed to the `GenericGadget2_Header` constructor.
            These may include specific header fields or overrides for default values.

        Returns
        -------
        GenericGadget2_Header
            A new instance of `GenericGadget2_Header` populated with data derived from the particle
        ...
        .. code-block:: python
            ...
            header = GenericGadget2_Header.from_particles(
                particles,
                particle_registry,
...
GenericGadget2_Header.BINARY_TO_HDF5 = GenericGadget2_Header.generate_field_mapping()
GenericGadget2_Header.HDF5_TO_BINARY = {
    v: k for k, v in GenericGadget2_Header.BINARY_TO_HDF5.items()
}


class GenericGadget2_ParticleRegistry(ABC):
    ...
    DEFAULT_PARTICLE_MAP: dict[int, str | None] = _MissingParameter()
...
class GenericGadget2_FieldRegistry(ABC):
    ...
    **Handling Custom Field Configurations**
    ...
    DEFAULT_FIELD_MAP: OrderedDict[str, dict[str, Any]] = _MissingParameter()
...
class GenericGadget2_ICFile(ABC):
    ...
    **Subclasses and Extensibility**
    ...
    def __init__(
        self,
        path: Union[str, Path],
        particle_registry: GenericGadget2_ParticleRegistry = None,
        field_registry: GenericGadget2_FieldRegistry = None,
    ):
        ...
        self._header: Union[GenericGadget2_Header, None] = None
        self._particle_registry: Union[
            GenericGadget2_ParticleRegistry, None
        ] = particle_registry
        self._field_registry: Union[GenericGadget2_FieldRegistry, None] = field_registry
    ...
    @property
    def header(self) -> GenericGadget2_Header:
        ...
        if self._header is None:
            self._header = GenericGadget2_Header.load_header(
                self.path, self.__class__._format
            )
        return self._header
    ...
    @property
    def particle_registry(self) -> GenericGadget2_ParticleRegistry:
        ...
        if self._particle_registry is None:
            self._particle_registry = GenericGadget2_ParticleRegistry({})
        return self._particle_registry

    @particle_registry.setter
    def particle_registry(self, value: GenericGadget2_ParticleRegistry):
        """Set the particle registry.

        Parameters
        ----------
        value : GenericGadget2_ParticleRegistry
            The new particle registry to assign.
        """
        self._particle_registry = value

    @property
    def field_registry(self) -> GenericGadget2_FieldRegistry:
        ...
        if self._field_registry is None:
            self._field_registry = GenericGadget2_FieldRegistry({})
        return self._field_registry

    @field_registry.setter
    def field_registry(self, value: GenericGadget2_FieldRegistry):
        """Set the field registry.

        Parameters
        ----------
        value : GenericGadget2_FieldRegistry
    ...
    @classmethod
    def from_particles(
        cls: Type["GenericGadget2_ICFile"],
        particles: ClusterParticles,
        path: Union[str, Path],
        field_registry: GenericGadget2_FieldRegistry = None,
        particle_registry: GenericGadget2_ParticleRegistry = None,
        header: GenericGadget2_Header = None,
        overwrite: bool = False,
        **kwargs,
    ):
        ...
        header : GenericGadget2_Header, optional
        ...
        **kwargs : dict, optional
        ...
        if field_registry is None:
            field_registry = GenericGadget2_FieldRegistry({})
...
class GenericGadget2_Binary(GenericGadget2_ICFile, ABC):
    ...
    def load_particles(self, inplace: bool = True) -> "ClusterParticles":
        ...
        .. code-block:: python
            ...
            >>> gadget_binary = GenericGadget2_Binary('path/to/gadget_file.g2')
        ...
        with open(self.path, "rb") as f:
            ...
            header_struct = GenericGadget2_Header._get_header_struct()

            if self._header is None:
                header_data = read_record(f, header_struct)
                header_dict = {
                    GenericGadget2_Header.BINARY_TO_HDF5[name]: header_data[name][0]
                    for name in header_struct.names
                    if name != "unused"
                }
                self._header = GenericGadget2_Header(**header_dict)
                devlog.debug(
    ...
    @classmethod
    def _write_particle_data(
        cls: Type["GenericGadget2_Binary"],
        path: str | Path,
        particles: "ClusterParticles",
        field_registry: "GenericGadget2_FieldRegistry",
        particle_registry: "GenericGadget2_ParticleRegistry",
    ...
    def get_block_struct(
        cls,
        block_name: str,
        registry: "GenericGadget2_FieldRegistry",
        header: "GenericGadget2_Header",
    ) -> np.dtype:
        """Construct the block struct based on header information and field registry
        data.
        ...
        .. code-block:: python
            ...
            >>> dtype = GenericGadget2_Binary.get_block_struct('POS ', field_registry, header)
...
class GenericGadget2_HDF5(GenericGadget2_ICFile, ABC):
    _format = "hdf5"
    _group_prefix = _MissingParameter()

    def __init__(self, *args, **kwargs):
        super(GenericGadget2_HDF5, self).__init__(*args, **kwargs)
    ...
    def _write_particle_data(
        ...
        for particle_id, cg_particle_name in particle_registry.map.items():
            if (cg_particle_name is None) or (
                particles.num_particles.get(cg_particle_name, 0) == 0
            ):
                continue
