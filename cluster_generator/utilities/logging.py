    ...
    def __get__(self, instance: Instance, owner: Type[Instance]) -> logging.Logger:
        ...
        if len(logger.handlers) > 0:
            ...
            _handler = logging.StreamHandler(
                getattr(sys, cgparams.config.logging.mylog.stream)
            )
            _handler.setFormatter(
                logging.Formatter(cgparams.config.logging.code.format)
            )
            logger.addHandler(_handler)
            logger.setLevel(cgparams.config.logging.code.level)
            logger.propagate = False
            logger.disabled = not cgparams.config.logging.code.enabled
